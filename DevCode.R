...
##
### Cas chronologique: les poissons du Doubs
if(FALSE) {
...
##
### Un autre cas 2D: les Scotch Whiskey
if(FALSE) {
    load("./constr.hclust/data/ScotchWhiskey.rda")
    ## L'exemple commence ici...
...
##
### Un autre chrono: les poissons de Tiahura
library(magrittr)
library(spdep)
##
if(FALSE) {
    load(file="DataRaw/Tiahura/tiahura.rds")
    ## str(tiahura)
    ## tiahura$Species
    tiah.jac <- tiahura$fish %>% t %>% ade4::dist.binary(method=1L)
    tiah.chclust <-
        constr.hclust(d=tiah.jac, coords=tiahura$Habitat[,"distance"],
                      chron=TRUE)
    par(mar=c(5,7,2,2),mfrow=c(1L,1L))
    plot(NA,xlim=tiahura$Habitat[,"distance"]%>%range,ylim=c(0.5,4.5),yaxt="n",
...
##
### Pas vraiment d'exemple valable à partir d'ici
##
if(FALSE) {
    m <- 5L
    col <- rainbow(m+1L)
    for(i in c("ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "centroid", "median", "flexible")) {
        ## i="ward.D"
        tmp <- constr.hclust(D.dat, method=i, neighbors)
        plot(x=coord.dat[,1L],y=coord.dat[,2L],main=i,xlab="x",ylab="y",asp=1,type="n")
        for(i in 1:nrow(neighbors)) lines(rbind(coord.dat[neighbors[i,1],], coord.dat[neighbors[i,2],]))
        points(x=coord.dat[,1L],y=coord.dat[,2L],pch=21L,cex=3,bg=col[cutree(tmp,m)])
        if(is.null(locator(1L))) break
    } ; rm(col,i,tmp)
}
##
### Un exemple simulé de grande taille:
if(FALSE) {
    coord.big <-
        cbind(
            (50*cos(pi*2*(1:6)/6)) %>%
...
##
if(FALSE) {
    load("./DataRaw/Oribates/mite.rds")
    ##
    mite.chclust <- constr.hclust(
        d=dist(vegan::decostand(mite$fau,method="hellinger")),
        links=mite$link,
        coords=as.matrix(mite$xy))
    ...
    mite.clshclust <- constr.lshclust(
        x=vegan::decostand(mite$fau,method="hellinger"),
        links=mite$link,
        coords=as.matrix(mite$xy))
    ##
    for(i in 1L:length(parts)) {
    ...
    for(i in 1L:69L) {
        res[i,1L] <- tail(
                getSS(vegan::decostand(mite$fau,method="hellinger"),mite.clshclust,i+1L),1L)
        res[i,2L] <- tail(
            getSS(vegan::decostand(mite$fau,method="hellinger"),mite.chclust,i+1L),1L)
    }
    cbind(res,res[,1L]-res[,2L],2:70)
    ##
    ##
    ##
    ## diff(mite.chclust$height)
    ## diff(mite.clshclust$height)
